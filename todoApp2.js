...
inputTask.addEventListener("click", function(e) {
    ...
    const update = e.target.classList.contains("update");
    if (target) {
        let val = e.target.parentElement.firstChild.textContent;
        // alert(val);
    ...
    if (update) {
        ...
        e.target.parentElement.innerHTML = `
    <li class="list-item"><span>${updatedValue}</span><button onclick="deleteTask2(this)">delete</button><button class="edit">edit</button></li>
    `;
    }
});
